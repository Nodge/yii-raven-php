...
class Raven_Client {
	...
	function __construct($options_or_dsn = null, $options = array()) {
		...
		if (empty($options['server_name']) && !empty($options['name'])) {
			$options['server_name'] = $options['name'];
		}
		...
		$this->name = (!empty($options['server_name']) ? $options['server_name'] : Raven_Compat::gethostname());
		...
		$this->logger = (!empty($options['logger']) ? $options['logger'] : 'root');
		$this->modules = (!empty($options['modules']) ? $options['modules'] : array());
		$this->extra = (!empty($options['extra']) ? $options['extra'] : array());
		$this->level = (!empty($options['level']) ? $options['level'] : null);
		$this->user = array();
...
//        }
	 ...
	 * @return type
	 */
	public function captureMessage($message, $options = array(), $stack = false) {
		if (!is_array($message)) {
			$message = array(
				'message' => $message,
				'params' => array(),
		...
		$data['message'] = vsprintf($message['message'], $message['params']);
	...
	public function capture($data, $stack) {
		...
		if (!empty($data['tags'])) {
			$data['tags'] = array_merge($this->tags, $data['tags']);
		}
		else {
			if (!empty($this->tags)) {
				$data['tags'] = $this->tags;
		...
		if (!isset($data['logger'])) $data['logger'] = $this->logger;

		if (!empty($data['modules'])) {
			$data['modules'] = array_merge($this->modules, $data['modules']);
		}
		else {
		...
		if (!empty($data['extra'])) {
			$data['extra'] = array_merge($this->extra, $data['extra']);
		}
		else {
			if (!empty($this->extra)) {
		...
		if (!empty($this->user) && isset($this->user['id'], $this->user['is_authenticated'])) {
